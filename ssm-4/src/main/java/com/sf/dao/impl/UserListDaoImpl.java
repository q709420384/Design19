...
public class UserListDaoImpl implements UserListDao {
	...
	public userEntity userlist(String userName) {
		//根据名字查询出用户对象. - - 
			...
			try {
				userEntity = qr.query(sql, new BeanListHandler<userEntity>(userEntity.class),userName);
	...
	public List<videoEntity> videolist(String videocAtegory) {
		...
		String sql="select * from video where videocAtegory=? order by rand() limit 8";//将视频标记为1的正常视频全部查询出
				...
				try {
					list = qr.query(sql, new BeanListHandler<videoEntity>(videoEntity.class),videocAtegory);
				} catch (SQLException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
	...
	public List<videoEntity> videolistimit7() {
		...
		List<videoEntity> list = null;
		try {
			list = qr.query(sql, new BeanListHandler<videoEntity>(videoEntity.class));
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
